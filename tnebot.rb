...
urlList.each do |url|
  open(url) do |rss|
    ...
    oldDate = DateTime.parse(db.execute(
      "SELECT date FROM `rss_feeds` WHERE url = ?;", url).to_s)
    if date > oldDate
      ...
      db.execute("UPDATE `rss_feeds` SET date = ? WHERE url = ?;",
        [date.to_s, url])
    end
  end
end
...
feedTitles.each do |title|
  feeds[title].each do |item|
    ...
    if twitter === true
      twitter_access_token = prepare_access_token(
        twitter_api_key,
      ...
      twitter_response = twitter_access_token.request(
        :post,
